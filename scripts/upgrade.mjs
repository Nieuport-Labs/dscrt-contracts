...
async function main() {
  ...
  /*
   * Nothing here rewrites a client's configuration any more.
   *
   * This used to overwrite the frontend's local env file, because printing the new hash
   * and trusting whoever ran the script to copy it is exactly what went wrong the first
   * time. The frontend then moved to its own repository, which would have made that write
   * land in a copy nobody deploys - while still reporting "Updated", which is the worst
   * of both.
   *
   * It is no longer needed: both clients resolve the code hash from the chain, so a
   * migration reaches them without anyone editing a file.
   */

  console.log(`\nThe code hash is now ${next.codeHash}`);
  console.log("\nBoth the frontend and the keeper resolve this from the chain, so neither needs");
  console.log("anything done to it. Only a deployment that deliberately pinned the hash has");
  console.log("to be updated - and a pin left set through a migration is the usual way a");
  console.log("working deployment breaks:");
  console.log("\n  NEXT_PUBLIC_CORE_CODE_HASH   frontend, compiled in, needs a rebuild");
  console.log("  LST_CORE_CODE_HASH           keeper, needs a restart");
}
